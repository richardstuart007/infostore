...
export default async function AdminEntryDetailPage({
  ...
  params: Promise<{ admin_secret: string; ent_entid: string }>
}) {
  const { admin_secret, ent_entid } = await params
  const entid = parseInt(ent_entid)

  const entry = await fetchEntryById(entid, 'AdminEntryDetailPage')
  if (!entry) {
    notFound()
  }

  const [arguments_, sources] = await Promise.all([
    fetchArgumentsByEntry(entid, 'AdminEntryDetailPage'),
    fetchSourcesByEntry(entid, 'AdminEntryDetailPage')
  ])
    ...
    <div className='max-w-4xl mx-auto space-y-8'>
      <div className='flex items-start justify-between'>
        ...
        <div className='flex gap-2'>
          <MyLink
            href={{
              pathname: `/${admin_secret}/dashboard/entries/${entid}/edit`,
